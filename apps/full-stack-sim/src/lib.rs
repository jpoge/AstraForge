...
use simple_gnc::{
    ...
    MagnetometerReading, NavigationSolution, SharedGncComponent, GNC_COMPONENT_ID, GNC_SOLUTION_TOPIC,
};
...
impl FullStackSim {
    pub fn new(config: SimConfig) -> Result<Self, SdkError> {
        ...
        let sensors = sensor_from_truth(initial_time, truth, MissionPhase::PadInitialization, &BTreeSet::new());
    ...
    fn advance_truth(&mut self, dt_s: f64) {
        ...
        let mut body_rates = [0.0, 0.0, 0.0];
        ...
        match self.phase {
            ...
            MissionPhase::Launch => {
                accel_nav = [
                    if self.anomalies.contains(&SimAnomaly::EngineFailure) { 1.2 } else { 2.6 },
                    0.15,
                    if self.anomalies.contains(&SimAnomaly::EngineFailure) { 6.0 } else { 18.0 },
                ];
                body_rates = [0.03, 0.02, 0.12];
            }
            MissionPhase::Flight => {
                accel_nav = [0.1, 0.0, -STANDARD_GRAVITY_MPS2];
                body_rates = [0.01, -0.01, 0.05];
            }
            MissionPhase::Landing => {
                accel_nav = [0.0, 0.0, if self.anomalies.contains(&SimAnomaly::HardLanding) { -12.0 } else { -3.0 }];
                body_rates = [0.0, 0.0, 0.01];
    ...
    fn refresh(&mut self, phase_changed: bool) -> Result<(), SdkError> {
        ...
        self.sensors = sensor_from_truth(now, self.truth, self.phase, &self.anomalies);
...
fn sensor_from_truth(
    ...
    SensorSnapshot {
        ...
        battery_current_a: match phase {
            MissionPhase::Launch => 16.0,
            MissionPhase::Flight => 12.0,
            MissionPhase::Landing => 9.0,
...
#[cfg(test)]
mod tests {
    use super::{FullStackSim, MissionPhase, PhaseControl, SimAnomaly, SimConfig, SimulatorCommand};
